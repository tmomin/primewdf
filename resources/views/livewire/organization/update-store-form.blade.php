<div>
    {{-- @if (Gate::check('addTeamMember', $team)) --}}
        ...
        <div class="mt-10 sm:mt-0">
            <x-jet-form-section submit="addStore">
                <x-slot name="title">
                    {{ __('Add Store') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('Add a new store to the organization') }}
                </x-slot>

                <x-slot name="form">
                    <div class="col-span-6">
                        <div class="max-w-xl text-sm text-gray-600">
                            {{ __('Please provide the store name you would like to use.') }}
                        </div>
                    </div>

                    <!-- Store Name -->
                    <div class="col-span-6 sm:col-span-4">
                        <x-jet-input id="store_id" type="hidden" wire:model.defer="addStoreFrom.id" />
                        <x-jet-label for="store_name" value="{{ __('Store Name') }}" />
                        <x-jet-input id="store_name" type="text" class="mt-1 block w-full" wire:model.defer="addStoreForm.name" />
